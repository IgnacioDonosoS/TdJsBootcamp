...
//3: Imprimir Consulas de Dental de forma especifica.
horasDental = dental.map(
  (item) =>
    `${item.hora} -  ${item.especialista}  -  ${item.paciente}  -  ${item.rut} - ${item.prevision}`
);
horasDental.forEach((element) => {
  document.write(`<p>${element}</p>`);
});
//4:
var todosJuntos = dental.concat(traumatologia, radiologia);
var todosNombres = todosJuntos.map((item) => item.paciente);
todosNombres.forEach((element) => {
  document.write(`<p>${element}</p>`);
});

var todosJuntos = [];
todosJuntos.push(radiologia);
todosJuntos.concat(dental);
todosJuntos.concat(traumatologia);

todosNombres = [];
for (let i = 0; i < todosJuntos.length; i++) {
  todosNombres.push(todosJuntos[i].map((item) => item.paciente));
}
for (let i = 0; i < todosNombres.length; i++) {
  todosNombres[i].forEach((element) => {
    document.write(`<p>${element}</p>`);
  });
}
...
var dentalIsapre = dental.filter((item) => item.prevision == "ISAPRE");
dentalIsapre.forEach((element) => {
  document.write(`<p>${element.paciente} - ${element.prevision}</p>`);
});
//6 Filtrar pacientes Fonasa de Traumatología

var trauFonasa = traumatologia.filter((item) => item.prevision == "FONASA");
trauFonasa.forEach((element) => {
  document.write(`<p>${element.paciente} - ${element.prevision}</p>`);
});
